...
VAR_BYTES * convert_data_ieb100(void *pure_data, int data_size)
{
	...
	LPWRITE_IEB100_PACKET lpwrite_ieb100_packet = make_write_ieb100_packet(0x7, 0xF0, 0x3, pure_data, data_size, &packet_ieb100_size);
...
{	
	...
	int nret = do_normal_process(inst, p1, p2, data, data_size, &precvbuff);
...
int check_sign_struct(SIGN_OPTION sign_option, int structure_size)
{
	...
	case SIGN_ECDSA_SHA256:
		if (structure_size != sizeof(ST_SIGN_ECDSA)){
			return ERR_SIGN_MODE_PARSE_ERR;
		}
		break;
	case SIGN_HMAC:
...
int check_dynamic_auth_struct(DYNAMIC_AUTH verify_etc, int structure_size)
{
	...
	//switch (verify_etc)
	//{
	//case CERT_PUB_SIGN_ECDSA:
	//case CERT_PUB_SIGN_ECDSA_SHA256:
	//case DYN_AUTH_SIGN_ECDSA_SHA256:
	//case DYN_AUTH_CERT_PUB_SIGN_ECDSA_SHA256:
	//	if (structure_size != sizeof(ST_SIGN_ECDSA)){
	//		return ERR_SIGN_MODE_PARSE_ERR;
	//	}
	...
	//case DYN_AUTH_SIGN_HMAC:
	//	if (structure_size != sizeof(ST_SIGN_HMAC)){
	//		return ERR_SIGN_MODE_PARSE_ERR;
	//	}
	//	break;
	//case DYN_AUTH_SIGN_SYMM:
	//	if (structure_size != sizeof(ST_SIGN_SYMM)){
	//		return ERR_SIGN_MODE_PARSE_ERR;
	//	}
	//	break;
